...
class ErrorsHandler {
  ...
  static Future<AppResponse> exceptionThrower(RequestFunction function) async {
    ...
    try {
      ...
      if (!isConnected) {
        L.warning('Request blocked: No network connectivity');
      ...
      if (_isNetworkRelatedDioException(e)) {
        L.warning('Network-related DioException detected: ${e.type}');
        throw NoInternetException();
      }
      ...
      L.warning('Unhandled DioException, treating as network error');
      throw NoInternetException();
    } catch (e, stackTrace) {
      L.detailedError(
